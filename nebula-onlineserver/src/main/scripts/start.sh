...
start(){
    ...
    then
         ...
         java ${JAVA_OPS} -classpath /etc/nebula/:/etc/nebula/online/:/etc/nebula/babels/:$(echo ${APP_HOME}/lib/* | tr ' ' ':') ${MAIN_CLASS} 1>${APP_HOME}/logs/log.txt 2>${APP_HOME}/logs/err.txt &
        pid=$(_pid)
        if [ -n "$pid" ]
        then
